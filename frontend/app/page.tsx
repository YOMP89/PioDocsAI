...
import { Eye, EyeOff, GraduationCap, Loader2, Lock, LogIn, User } from 'lucide-react'
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (window.localStorage.getItem('piodocsai_sesion')) router.replace('/dashboard')
  }, [router])
  ...
  async function manejarSubmit(e: React.FormEvent) {
    ...
    setCargando(true)
    try {
      const sesion = await api.login(usuario.trim(), password)
      window.localStorage.setItem('piodocsai_sesion', JSON.stringify(sesion))
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-xl shadow-blue-900/5">
        ...
        <form onSubmit={manejarSubmit} className="mt-7 space-y-4">
          ...
          <button
            type="submit"
            disabled={cargando}
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-linear-to-br from-blue-600 to-blue-700 py-3 text-sm font-semibold text-white shadow-md shadow-blue-600/25 transition hover:from-blue-700 hover:to-blue-800 active:from-blue-800 active:to-blue-900 disabled:cursor-not-allowed disabled:opacity-70"
          >
